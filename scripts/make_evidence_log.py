...
with open(OUT, "w") as f:
    ...
    for d in ORDER:
        ...
        for grp in GROUPS:
            ...
            if not g: continue
            f.write(f"### {grp}\n\n")
            f.write("| Attack class (paper wording) | Technique | Tactic | Conf. | Paper location | Verbatim evidence | Analyst note |\n")
            f.write("|---|---|---|---|---|---|---|\n")
            for r in g:
                ...
                note = r.get("analyst_note","").replace("|","/").replace("\n"," ")
                f.write(f"| {r['attack_class_paper_wording']} | {tech} | {r['tactic'] or '—'} | {r['confidence']} | {loc} | \"{ev}\" | {note} |\n")
            f.write("\n")
        f.write("---\n\n")
