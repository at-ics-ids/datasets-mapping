...
etac = collections.defaultdict(list)
for _r in csv.DictReader(open(os.path.join(DATA, "enterprise_tactics.csv"))):
    etac[_r["technique_id"]].append(_r["enterprise_tactic"])
...
ent_by_tactic = collections.Counter(tac for t in ent_ids for tac in etac[t])
...
with open(p3, "w", newline="") as f:
    ...
    w.writerow(["Enterprise","Enterprise total", len(ent_ids), "n/a", "n/a"])
...
print(f"  Enterprise techniques (separate): {len(ent_ids)}")
