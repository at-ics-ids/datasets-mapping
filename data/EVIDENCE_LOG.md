...
| Attack class (paper wording) | Technique | Tactic | Conf. | Paper location | Verbatim evidence | Analyst note |
|---|---|---|---|---|---|---|
...
| Information gathering (port scan) | T0846.001 Remote System Discovery: Port Scan | Discovery | high | Ferrag et al. 2022 Sec.B-2 | "obtain the composition of a network's architecture" | weakest of the three; port/connection enumeration |
...
| Malware (backdoor) | T1219 Remote Access Tools | Enterprise | enterprise | Ferrag et al. 2022 Sec.B-5 + ATT&CK Enterprise | "installing backdoors to take control of vulnerable IoT network components" |  |
...
| Reconnaissance (port scan) | T0846.001 Remote System Discovery: Port Scan | Discovery | high | Zhou et al. 2026 Sec.-1 | "...and open ports... port scans" |  |
...
| Exploitation (reverse shell/backdoor) | T1219 Remote Access Tools | Enterprise | enterprise | Al-Hawawreh et al. 2022 Sec.III-B-3 + ATT&CK Enterprise | "create a persistent reverse TCP shell/backdoor in the edge gateway" |  |
...
| Attack class (paper wording) | Technique | Tactic | Conf. | Paper location | Verbatim evidence | Analyst note |
|---|---|---|---|---|---|---|
| DROPPED: cloud-data poisoning mislabelled (T1692.002 Reporting Msg) |  | — | removed | Al-Hawawreh et al. 2022 Sec.III-B-7 | "Cloud-data poisoning targets the cloud ML pipeline ("poisoning of cloud data (i.e., false data injections)... to affect the accuracy of a cloud data analysis"), NOT a forged OT reporting message. Same basis as the ICS-NAD FDI removal. T1692.002 unsupported." |  |
...
| Reconnaissance (Port-Scanning) | T0846.001 Remote System Discovery: Port Scan | Discovery | high | Dehlaghi-Ghadim et al. 2023 Sec.IV-1 | "Port-Scanning... we use NMap... to find hosts and ports on them in the network" |  |
...
| Replay | T1692.001 Unauthorized Message: Command Message | Impair Process Control | high | Dehlaghi-Ghadim et al. 2023 Sec.IV-2 | "a replay attack on a TCP connection reuses only TCP payloads, Modbus commands, and arguments to create a new TCP-Connection" |  |
...
| MITM | T0830 Adversary-in-the-Middle | Collection | high | Faramondi et al. 2021 Table 2 + Fig.6 | "MITM attacks... attacks against hosts or communication links" | SCADA, PLC, flow sensor |
...
| DoS | T0814 Denial of Service | Inhibit Response Function | high | Morris & Gao 2014 Sec.4.4 | "Denial-of-service attacks target communications links and system programs in an attempt to exhaust resources" |  |
...
| sensor spoofing (falsify controller/operator view) | T0832 Manipulation of View | Impact | high | Goh et al. 2016 Sec.4 (Fig.5, spoofed LIT301 sensor) | "This deceives the PLC to think that there is an over supply of water and turns the pump on to supply water to P4" | spoofed sensor value falsifies the PLC/operator view of the process |
...
| Attack class (paper wording) | Technique | Tactic | Conf. | Paper location | Verbatim evidence | Analyst note |
|---|---|---|---|---|---|---|
| second-stage malware download [A6] | T1105 Ingress Tool Transfer | Enterprise | enterprise | SWaT.A6 Dec 2019 Log.docx (iTrust) + ATT&CK Enterprise | "Infiltrate SCADA WS with second malware, via downloading from C2 server (A6 Dec 2019, ~1230)" |  |
