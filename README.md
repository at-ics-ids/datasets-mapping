...
**Headline.** The ten datasets exercise **17 ICS techniques** across **7 of the 12 tactics**.
Information-technology attacks carried by some datasets map to **8 ATT&CK Enterprise techniques**, reported
...
describes; **medium** (8 cells) means the technique follows by inference from that description. Of the 68 ICS
assignments, one technique (`T0826 Loss of Availability`) rests on a single medium cell; rejecting every medium
assignment would leave 16 techniques and would empty no tactic.

Six candidate assignments were considered and rejected; they are kept in `data/mapping_evidence.csv` with
